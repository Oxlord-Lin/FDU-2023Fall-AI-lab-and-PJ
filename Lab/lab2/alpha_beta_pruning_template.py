...
def get_value(node, alpha, beta):
    ...
    if node.rule == 'max':
        # print(max_value(node,alpha,beta))
        return max_value(node,alpha,beta)
    else: # node.rule == 'min'
        # print(min_value(node,alpha,beta))
...
def max_value(node, alpha, beta):
    ...
    for suc in node.successor:
        suc.visited = True
        v = max(v, get_value(suc,alpha,beta))
        if v >= beta:
            return v
        alpha = max(alpha,v)
    return v
...
def min_value(node, alpha, beta):
    ...
    for suc in node.successor:
        suc.visited = True
        v = min(v, get_value(suc, alpha,beta))
        if v <= alpha:
            return v
        beta = min(beta,v)
    return v
